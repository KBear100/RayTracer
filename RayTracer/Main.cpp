...
int main(int, char**)
{
	...
	Scene scene({ 1, 0.5, 0.5 }, { 0.5, 0.5, 1 });
	InitScene02(scene);
...
{
	//light
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 5, 5, 3 }, 1.0f, std::make_unique<Emissive>(color3{ 40, 40, 40 })));

	//objects
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 0, 0, -1 }, 0.5f, std::make_unique<Lambertian>(color3{ 0, 1, 0 })));
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ 1.3, 0, -1 }, 0.5f, std::make_unique<Metal>(color3{ 1, 1, 1 }, 0.3f)));
	scene.AddObject(std::make_unique<Sphere>(glm::vec3{ -1.3, 0, -1 }, 0.5f, std::make_unique<Dielectric>(color3{ 1, 1, 1 }, 2.0f)));

	scene.AddObject(std::make_unique<Plane>(glm::vec3{ 0, -0.4, 0 }, glm::vec3{ 0, 1, 0 }, std::make_unique<Lambertian>(color3{ 0.2, 0.2, 0.2 })));
}
